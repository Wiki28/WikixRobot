...
async def logo_(event):
    ...
    if event.reply_to_msg_id:
        ...
        pics = []
        async for i in ubot2.iter_messages(
            "@WikiLogoCecan", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Wiki W](https://t.me/WikiTapiRobot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
...
❂ /logo <text/name> - Buat logo dengan tampilan acak.
❂ /wlogo <text/name> - Buat logo dengan tampilan lebar saja.
❂ /clogo <text/name> - Buat logo dengan tampilan cewe cantik
 Image Editor :
❂  /edit <reply photo> - untuk mengedit gambar.
"""
